 ...
 * @Description : 一个扩展的通用适配器，处理列表项的显示和绑定。该适配器使用 ViewBinding 来高效处理视图。
 *
 * @param T 显示在列表中的项的类型。
 * @param VB 用于绑定视图的 ViewBinding 类型。
 * @param diffCallback 用于计算列表之间差异的 DiffUtil.ItemCallback 实例。
 */
abstract class BaseListPlusAdapter<T, VB : ViewBinding>(
     ...
     * 该方法也会接收 ViewHolder，以便在绑定时能够访问到 ViewHolder 的其他属性。
     *
     * @param holder 要绑定数据的 ViewHolder 实例。
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<VB>, position: Int) {
        ...
        bind(holder, holder.binding, item, position)
    }
